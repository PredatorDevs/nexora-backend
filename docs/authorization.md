...
The multi-company migration is implemented: permissions declare a scope,
company roles are private per company, memberships receive constrained roles,
and authorization derives company context from the authenticated session.
...
`authorizeCompany` requires an active tenant context and rejects a route
`companyId` that differs from the session company before the service executes.
It resolves permissions by `(membershipId, companyId)`. `authorizePlatform`
resolves only `PLATFORM` permissions from global user roles, even when that user
currently has a company selected.

The legacy `authorize` export remains a temporary alias of `authorizePlatform`
for existing platform administration routes.
