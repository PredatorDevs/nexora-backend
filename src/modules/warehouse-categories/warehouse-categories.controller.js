...
export function createWarehouseCategoriesController(service, auditService) {
  ...
  const audited = (request, event, operation) => auditService
    ? auditService.execute({
        actorUserId: request.auth.userId,
        resourceType: 'warehouse_category',
  ...
  return {
    async list(request, response) {
      ...
      return sendSuccess(response, result.warehouseCategories, { meta: { pagination: result.pagination } });
    },
    ...
    async create(request, response) {
      return sendSuccess(response, await audited(request, {
        action: auditActions.warehouseCategoryCreated,
    ...
    async update(request, response) {
      return sendSuccess(response, await audited(request, {
        action: auditActions.warehouseCategoryUpdated,
    ...
    async changeStatus(request, response) {
      return sendSuccess(response, await audited(request, {
        action: auditActions.warehouseCategoryStatusChanged,
        resourceId: request.validated.params.id,
