...
const coordinate = z.string().trim().min(1).max(50).transform((value) => value.toUpperCase());
const capacity = z.coerce.number().positive().max(99999999999999).nullable().optional();
const capacityUnit = z.enum(['UNITS', 'KG', 'M3', 'PALLETS']).nullable().optional();
...
export const locationIdParams = z.object({ id: z.coerce.number().int().positive() });
export const locationsListQuery = createListQuerySchema([
  'createdAt', 'code', 'aisle', 'rack', 'level', 'position', 'isActive',
]).extend({
