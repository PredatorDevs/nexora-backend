...
export function createLocationsController(service, auditService) {
  ...
  return {
    async list(request, response) {
      ...
      return sendSuccess(response, result.locations, {
    ...
    async createBulk(request, response) {
      return sendSuccess(
        ...
        await audited(
          request,
          {
            action: auditActions.locationsBulkCreated,
            resourceId: null,
            metadata: {
              levelCount: request.validated.body.levelCount,
              positionsPerLevel: request.validated.body.positionsPerLevel,
            },
          },
          () =>
            service.createBulk(
