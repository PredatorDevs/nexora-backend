  ...
  return {
    ...
    findCoordinateConflicts(
      ...
      return client.location.findMany({
        where: { companyId, warehouseId, OR: coordinates },
        select: { aisle: true, rack: true, level: true, position: true },
      });
    },
    async createBulk(companyId, data, client = prisma) {
      await client.location.createMany({
        data: data.map((value) => ({ companyId, ...value })),
      });
      return client.location.findMany({
        where: { companyId, code: { in: data.map(({ code }) => code) } },
        select,
        orderBy: { code: 'asc' },
      });
    },
    async update(companyId, id, expectedUpdatedAt, data, client = prisma) {
      const result = await client.location.updateMany({
