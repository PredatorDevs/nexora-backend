...
export function createPurchaseOrdersRepository(prisma) {
  return {
    ...
    async create(data,client=prisma){const {details,expenses,...header}=data;const order=await client.purchaseOrder.create({data:header,select:{id:true}});if(details?.create?.length)await client.purchaseOrderDetail.createMany({data:details.create.map((detail)=>({...detail,purchaseOrderId:order.id}))});if(expenses?.create?.length)await client.purchaseOrderExpense.createMany({data:expenses.create.map((expense)=>({...expense,purchaseOrderId:order.id}))});return this.find(header.companyId,order.id,client);},
    completeRequest(companyId,purchaseRequestId,client=prisma){return client.purchaseRequest.updateMany({where:{id:purchaseRequestId,companyId,status:'IN_QUOTATION'},data:{status:'COMPLETED'}});},
