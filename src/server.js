...
import { createWarehousesRepository } from './modules/warehouses/warehouses.repository.js';
...
import { createBrandsRepository } from './modules/brands/brands.repository.js';
...
const warehousesService = createWarehousesService({
  repository: createWarehousesRepository(prisma),
  entityChangeService,
  runInTransaction,
});
const locationsService = createLocationsService({
  repository: createLocationsRepository(prisma),
  entityChangeService,
  runInTransaction,
});
...
const brandsService = createBrandsService({
  repository: createBrandsRepository(prisma),
  entityChangeService,
  runInTransaction,
});
const productCategoriesService = createProductCategoriesService({
  repository: createProductCategoriesRepository(prisma),
  entityChangeService,
  runInTransaction,
});
...
const purchaseRequestsService = createPurchaseRequestsService({
  repository: createPurchaseRequestsRepository(prisma),
  entityChangeService,
  runInTransaction,
});
...
const purchaseQuotationsService = createPurchaseQuotationsService({
  repository: createPurchaseQuotationsRepository(prisma),
  entityChangeService,
  runInTransaction,
});
...
const purchasesService = createPurchasesService({
  repository: createPurchasesRepository(prisma),
  entityChangeService,
  runInTransaction,
});
